namespace :db do
  ...
  task :rails_world_2025_seed, [:start_date] => :environment do |t, args|
    ...
    # Create Locations
    track_1 = conference.locations.find_or_create_by!(name: "Main Stage")
    ...
    david_heinemeier_hansson = Profile.find_or_create_by!(name: "David Heinemeier Hansson") do |profile|
      ...
      profile.image = Rails.root.join("#{image_path}/DHH.jpeg").open
      Speaker.find_or_create_by!(profile: profile)
    end.profileable
    ...
    katarina_rossi = Profile.find_or_create_by!(name: "Katarina Rossi") do |profile|
      ...
      profile.twitter_url = ""
      profile.image = Rails.root.join("#{image_path}/RW-Katarina_Rossi.jpg").open
      Speaker.find_or_create_by!(profile: profile)
    end.profileable
    ...
    masafumi_okura = Profile.find_or_create_by!(name: "Masafumi Okura") do |profile|
      ...
      profile.image = Rails.root.join("#{image_path}/TW-Masafumi_Okura.jpg").open
      Speaker.find_or_create_by!(profile: profile)
    end.profileable
